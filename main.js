...
function updateLoginButton() {
    ...
    if (currentUser) {
        ...
        logoutBtn.innerHTML = '<i class="fas fa-right-from-bracket"></i> Đăng xuất';
...
function applyResponsiveLayout() {
    ...
    const isTabletViewport = window.matchMedia('(min-width: 769px) and (max-width: 1024px)').matches;
    const isTouchDevice = window.matchMedia('(pointer: coarse)').matches || 'ontouchstart' in window || navigator.maxTouchPoints > 0;

    body.classList.toggle('device-mobile', isMobileViewport);
    body.classList.toggle('device-tablet', !isMobileViewport && isTabletViewport);
    body.classList.toggle('device-desktop', !isMobileViewport && !isTabletViewport);
    body.classList.toggle('device-touch', isTouchDevice);
}
